...
fix_storage() {
    [ -d "/sdcard" ] || {
        ...
        sleep 2
        [ ! -d "/sdcard" ] && {
            red "❌ 请手动授予存储权限：打开Termux设置→存储→允许访问"
            exit 1
        }
    }
}
...
select_file() {
    ...
    if command -v termux-file-editor >/dev/null 2>&1; then
        ...
        rm -f "$temp"
        # 转换为Termux可识别路径
        path=$(echo "$path" | sed "s|/storage/emulated/0|/sdcard|g")
    ...
    # 手动输入（带路径引导）
    yellow "📌 请输入文件路径（示例：/storage/emulated/0/boot.img）"
    while true; do
        read -p "📂 路径: " path
        # 自动补全/sdcard前缀
...
check_device() {
    local adb_dev=$(adb devices | grep -v "List" | grep -v "^$")
    local fb_dev=$(fastboot devices | grep -v "^$")
